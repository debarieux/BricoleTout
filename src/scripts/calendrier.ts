...
let currentMonthDate: Date = new Date();
...
function createCalendarUI(container: HTMLElement) {
...
function selectDate(date: Date) {
  ...
  document.querySelectorAll('#calendar-grid div[data-date]').forEach(dayCell => {
    if ((dayCell as HTMLElement).dataset.date === formatDate(date)) {
...
function selectTimeSlot(slot: TimeSlot) {
  ...
  document.querySelectorAll('#slots-container div[data-slot-id]').forEach(slotElement => {
    if ((slotElement as HTMLElement).dataset.slotId === slot.id) {
