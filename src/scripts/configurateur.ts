...
let selectedServices: {service: ServiceOption, option?: SubOption}[] = [];
let totalPrice = 0;
...
function createConfiguratorUI(container: HTMLElement) {
...
function showServiceOptions(service: ServiceOption) {
  ...
  if (service.options && service.options.length > 0) {
    service.options.forEach(option => {
      ...
      optionItem.querySelector('.add-option-btn')?.addEventListener('click', (e) => {
        e.stopPropagation();
    ...
    baseOption.querySelector('.add-base-btn')?.addEventListener('click', () => {
...
function addServiceWithOption(service: ServiceOption, option?: SubOption) {
  // Vérifier si le service est déjà sélectionné
  const existingIndex = selectedServices.findIndex(s => 
  ...
  const requestQuoteBtn = document.getElementById('request-quote-btn') as HTMLButtonElement;
  if (requestQuoteBtn) {
...
function updateSummary() {
  ...
  if (!selectedServicesList || !totalPriceElement) return;
